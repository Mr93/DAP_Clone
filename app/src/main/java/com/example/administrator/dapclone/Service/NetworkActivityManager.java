...
public class NetworkActivityManager extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		try {
			if (intent != null) {
				FileInfo fileInfo = (FileInfo) intent.getParcelableExtra(ConstantValues.FILE_INFO);
	...
	private void downloadAPart(final OkHttpClient client, final FileInfo fileInfo, final int startPosition) {
		...
		Call<ResponseBody> responseBodyCall = networkApi.download(fileInfo.url, range, "close");
	...
	private synchronized void writeToFile(ResponseBody body, FileInfo fileInfo, int position) throws IOException {
